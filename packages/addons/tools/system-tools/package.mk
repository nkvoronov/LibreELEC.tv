...
addon() {
  mkdir -p ${ADDON_BUILD}/${PKG_ADDON_ID}/{bin,lib,etc,share}
  ...
  if [ "${ENABLE_FILE}" = "yes" ]; then
    ...
    cp -P $(get_install_dir file)/usr/share/misc/magic.mgc ${ADDON_BUILD}/${PKG_ADDON_ID}/share
  fi
  ...
  if [ "${ENABLE_HDDTEMP}" = "yes" ]; then
    ...
    cp -P $(get_install_dir hddtemp)/usr/share/misc/hddtemp.db ${ADDON_BUILD}/${PKG_ADDON_ID}/etc/
  fi
  ...
  # mc
  if [ "${ENABLE_MC}" = "yes" ]; then
    cp -PR  $(get_install_dir mc)/usr/bin/* ${ADDON_BUILD}/${PKG_ADDON_ID}/bin/
    cp -Pa $(get_install_dir mc)/storage/.kodi/addons/tools.system-tools/etc/* ${ADDON_BUILD}/${PKG_ADDON_ID}/etc
    mkdir -p ${ADDON_BUILD}/${PKG_ADDON_ID}/share/mc
    cp -Pa $(get_install_dir mc)/storage/.kodi/addons/tools.system-tools/share/mc/* ${ADDON_BUILD}/${PKG_ADDON_ID}/share/mc
    mkdir -p ${ADDON_BUILD}/${PKG_ADDON_ID}/lib/mc
    cp -Pa $(get_install_dir mc)/storage/.kodi/addons/tools.system-tools/lib/mc/* ${ADDON_BUILD}/${PKG_ADDON_ID}/lib/mc
    mkdir -p ${ADDON_BUILD}/${PKG_ADDON_ID}/share/locale
    cp -Pa $(get_install_dir mc)/storage/.kodi/addons/tools.system-tools/share/locale/* ${ADDON_BUILD}/${PKG_ADDON_ID}/share/locale
  fi
  ...
  # screen
  if [ "${ENABLE_SCREEN}" = "yes" ]; then
    cp -P $(get_install_dir screen)/usr/bin/screen-4.9.0 ${ADDON_BUILD}/${PKG_ADDON_ID}/bin/screen
  fi
  ...
  if [ "${ENABLE_VIM}" = "yes" ]; then
    ...
    cp -Pa $(get_install_dir vim)/storage/.kodi/addons/tools.system-tools/share/* ${ADDON_BUILD}/${PKG_ADDON_ID}/share
  fi
}
